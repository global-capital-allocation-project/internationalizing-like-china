...
drop if currency == "BBB-" | cgs_domicile == "BBB-" | country_bg == "BBB-"
* adjustment for (very) few Development Bank issuances classified as corporates
replace asset_class2 = "Sovereign Bond" if regexm(lower(securityname),"china developm") & currency == "CNY" & asset_class2 == "Corporate Bond"
keep MasterPortfolioId DomicileCountryId FundName asset_class* currency cgs_dom country_bg domestic marketvalue_usd year
*keep MasterPortfolioId DomicileCountryId FundName asset_class* currency cgs_dom country_bg domestic marketvalue_usd year cusip isin
...
save "${gcap_data}/output/holdings_similarity/holdings_Equity_country_bg_`year'.dta", replace
...
drop interval_lower interval_upper se
drop if country_bg=="JEY"
...
* saving correlations 
save "${gcap_data}/output/holdings_similarity/correlations_Equity_country_bg_`year'.dta", replace
