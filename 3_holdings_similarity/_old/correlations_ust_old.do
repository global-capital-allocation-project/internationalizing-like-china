...
* read file
use "${gcap_data}/output/morningstar/output/HD_for_analysis/HD_`year'_y.dta", clear
...
qui gen group_currency = "DM" if currency=="USD" // UST only, because earlier we selected to keep only LC government, so only USA. 
...
save "${gcap_data}/output/holdings_similarity/holdings_LC_Govt_currency_`year'_ust.dta", replace
