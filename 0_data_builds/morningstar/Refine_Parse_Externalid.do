...
use  $temp/HoldingDetail/HoldingDetail_`year'_m_step1, clear
...
if !_rc {
	...
	* Clean up the externalid by excluding redundant text that appears before/after space or slash or underscore etc.
	 cap {
		split externalid_mns if regexm(externalid_mns, " ") | regexm(externalid_mns, "_") | regexm(externalid_mns, "/") | regexm(externalid_mns, "-"), ///
		parse(" " "_" "/" "-") limit(3) gen(externalid_mns_cln) 
...
save $temp/HoldingDetail/HoldingDetail_`year'_m_parseextid, replace
