...
*cap log close
*log using "$logs/Internal/${whoami}_Internal_Class", replace

* Append monthly files and use only relevant variables
clear
forvalues year=$firstyear/$lastyear {
    di "Appending HoldingDetail_`year'_m_cusipmerge.dta ..."
    capture append using "$temp/HoldingDetail/HoldingDetail_`year'_m_cusipmerge.dta", keep(cusip mns_class mns_subclass MasterPortfolioId)
}

* Perform the country assignment
local test=0
local app=""
if `test'==1 {
...
save "$output/Internal_Class/Internal_Class.dta", replace
