...
*cap log close
*log using "$logs/externalid/${whoami}_collect_externalid_master", replace
...
foreach year of numlist $firstyear/$lastyear {
	cap append using "${gcap_data}/output/morningstar/temp/HoldingDetail/HoldingDetail_`year'_m_cusipfisin.dta", keep(`vars_to_keep')
...
save "${gcap_data}/output/morningstar/temp/externalid/extid_records_allyears_summary.dta", replace
